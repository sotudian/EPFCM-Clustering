...
%  General information
%  ---------------------------------------------------------------------------------------------------------------------------------------
%	This code provides IT2 EPFCM algorithm described in "Interval and General Type-2 Enhanced Possibilistic Fuzzy C-Means clustering for Gene Expression Data Analysis"
%   
...
for i = 1:max_iter
    ...
    b1=((Cp*(Theta1^2)).*(dist1 .^ 2)) ./ ( Eta * ones (1, n));
    tmp1=((Theta1^2)-1)*b1;
    Power1=Theta1/((Theta1^2)-1);
    T_new1 = exp(-lambertw(tmp1)).^Power1;

    b2=((Cp*(Theta2^2)).*(dist2 .^ 2)) ./ ( Eta * ones (1, n));
    tmp2=((Theta2^2)-1)*b2;
    Power2=Theta2/((Theta2^2)-1);
    T_new2 = exp(-lambertw(tmp2)).^Power2;
