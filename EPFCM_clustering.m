...
%  General information
%  ---------------------------------------------------------------------------------------------------------------------------------------
%	This code provides EPFCM algorithm described in "Interval Type-2 Enhanced Possibilistic Fuzzy C-Means clustering for Gene Expression Data Analysis"
%   
...
for i = 1:max_iter,
    ...
    % Claculate new T
    b=((Cp*(Theta^2)).*(dist .^ 2)) ./ ( Eta * ones (1, n));
    tmp2=((Theta^2)-1)*b;
    Power=Theta/((Theta^2)-1);
    T = exp(-lambertw(tmp2)).^Power;
